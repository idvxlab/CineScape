...
class PromptBuilder:
    ...
    def discover_questions(
        ...
        system = (
            ...
            "一道偏好问题 q=(c,d,a,b):\n"
            "- c 上下文(scope):**从被选方向的 dominant_intents 里取**(见线索 comparisons[].selected\n"
            "  .dominant_intents 或 digest.dominant_intents)——选其中**与该偏好参数最直接相关**的那个意图\n"
            "  作 intent_leaf 的 scope_id(偏好来自这个选择,就归到该选择所服务的意图,而非会话里任何其它 tag)。\n"
            "  跨场景的一贯风格(如 movement 机位静止、color_tone 冷暖)不绑单一意图,用 global。也可用 mechanism。\n"
            "  ⚠ 不得把偏好归到被选方向未服务的意图,否则未来同类会话召不回、也无法被正确归因。\n"
