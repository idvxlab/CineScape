...
async def align_node(state: SessionState) -> dict:
    ...
    try:
        ...
        # 结构化判断节点无需思维链(client 文档亦如此建议):关 thinking 大幅提速,
        # 且 align 只出 JSON 控件与维度更新,质量不受影响。reasoning 便为空。
        result, reasoning = await client.chat(
            system, user, return_reasoning=True, enable_thinking=False
        )
    ...
    # 记忆探针的召回与注入已上移到 convergence 节点(ADR-0017):那里 tags 已确认,
    # 召回可靠且对快速收敛的会话也有效。align 不再依赖早期空的临时维度码。
    return {
